 ...
 * (the server injects the trusted socket identity). Coarse guard only; the engine
 * enforces role + clue legality (length, digits).
 */
export const sintoniaActionSchema = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('setGuess'), value: z.number().min(0).max(100) }),
  z.object({ type: z.literal('lockGuess') }),
  z.object({ type: z.literal('unlockGuess') }),
]);

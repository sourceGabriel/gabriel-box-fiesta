...
_Atualizado em 2026-09-07 (Fases A–D + avatares em `main` @ `0079344`; branch `feature/quiplaxi` — **jogo #3 "Zap!" COMPLETO** (Z1–Z5), 77 testes de servidor; 3 jogos no catálogo)._
...
**Zap! (jogo #3) COMPLETO** (Z1–Z5). §52 preservado o tempo todo: o jogo inteiro = `shared/{models,games}/zap`, `server/src/games/zap/`, `host/src/games/zap/`, `mobile/src/games/zap/`, 1 componente `@party/ui` (`TextAnswerInput`), +1 linha em cada um dos 3 registries. Zero edições em `core/`, `ws-server.ts`, shells ou `shared/protocol`. **3 jogos** no catálogo: UNO, Coup, Zap!.

...
## Next planned change
- Fase A, PR A3: extract the host into `host/src/shell/` + `host/src/games/uno/` (no wire change). Then A4 (mobile shell + session subsystem), A5 (drop the legacy UNO verbs, generic `{ gameId, state }` payloads).

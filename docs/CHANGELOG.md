...
### Added — code
- `server/src/tarefas/` — a personal side utility served off the same HTTP server, **not** a game feature:
  - `page.html` — the house-tasks organizer (grouped by tipo de ação / cômodo, drag-to-reorder, check-off, progress bar). Ported from `~/Documents/tarefas-casa.html` with a `window.storage` shim that talks to the server instead of Artifact storage.
  - `store.ts` — one JSON file (`server/data/tarefas-casa.json`, gitignored, `TAREFAS_DATA_FILE` env override), loaded once, rewritten on every change → the list survives server restarts.
  - `handler.ts` — `GET /tarefas` (the page) · `GET /tarefas/state/:key` → `{ value }` · `PUT|POST /tarefas/state/:key` persists `{ value: string }`.
- `server/src/websocket/ws-server.ts` — HTTP handler made async; `/tarefas*` routed to `handleTarefasRequest` before the game routes; CORS methods now `GET, PUT, POST, OPTIONS`; startup log prints `tarefasUrl` (`http://<lan-ip>:3001/tarefas`) next to `joinUrl`.
- `.gitignore` — `server/data/`.
### Why
- The user wanted the same "server prints a URL you open on any device on the LAN" flow the game has, but for a living to-do list whose state is really saved on disk between runs.
### Validation
- `tsc` server clean · 77 server tests green · 5-workspace build green.
- Live: `preview_start server` → `GET /tarefas` 200, check a task → `PUT /tarefas/state` 200 → `server/data/tarefas-casa.json` on disk carries `done:true` → reload reads it back (1/31). No console errors.
### Notes
- Outside the master-prompt spec (a personal tool, not a party game) — recorded here for history, not tracked in `docs/repository-gap-review.md`.
- §52 untouched: no `core/`, game engine, shell, or `shared/protocol` changes; the only `ws-server.ts` edit is HTTP routing for this non-game endpoint.

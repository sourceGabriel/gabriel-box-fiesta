...
**Autonomous run — DONE (2026-09-08):** dark / crude / +18 humour push across the party games. ✅ Lorota! merged (`8a06d03`). ✅ Pack adulto (+18) for Zap! & Lorota! merged (`a9158b6`). ✅ **Sabe-Tudo (game #5)** — multiple-choice trivia (59 PT-BR Qs incl. +18 pack, speed + streak scoring) merged (`da60fa6`). ✅ **FDP — Foi De Propósito (game #6)** — Cards-Against-Humanity-style (58 PT-BR +18 prompts, write-one-answer + vote-the-best + sweep bonus, `feature/fdp`). **6 games in the catalog, 120 server tests.** All full-slice live-smoked. Guardrails held: no protected group as punchline, no real private people, nothing sexual involving minors.

UNO MVP **complete and validated** (Fases 1–10 + mobile visual overhaul). **Coup added as game #2** (Fase D, classic ruleset) with **real character card art** (D6a). **Customizable LPC pixel avatars** added as a platform feature. **Zap! (game #3, Quiplash-inspired) COMPLETE on `feature/quiplaxi`** (Z1–Z5) — shared contract, server engine (77 tests), host view, mobile controller + `@party/ui TextAnswerInput`, sound + reveal animation. **Lorota! (game #4, Fibbage-inspired) COMPLETE on `feature/fibbage`** — bluffing trivia; both games full-slice smoke-tested end-to-end. **Sabe-Tudo (game #5, multiple-choice trivia) COMPLETE + merged (`da60fa6`). FDP — Foi De Propósito (game #6, Cards-Against-Humanity-style, +18) COMPLETE on `feature/fdp`.** **6 games in the catalog (UNO/Coup/Zap!/Lorota!/Sabe-Tudo/FDP).** UX polish (2026-09-08): lobby avatar re-edit (`UPDATE_AVATAR`) + cancelable UNO colour picker. Server tests: **120 green**. Build: 5 workspaces green.
...
**Next-games direction (2026-09-08):** roadmap in `~/.claude/plans/quiplax-e-proximos-jogos.md`. #3 **Zap!** ✅ (`feature/quiplaxi`, merged to `main`) → #4 **Lorota!** ✅ (`feature/fibbage`, merged) → **#5 "Sabe-Tudo"** ✅ (Trivia, merged) → **#6 "FDP — Foi De Propósito"** ✅ (`feature/fdp`, Cards-Against-Humanity-style, +18). Each new game = the §52 checklist (UNO/Coup/Zap!/Lorota!/Sabe-Tudo/FDP are the mould). **NO BOTS** — the user descoped bots / virtual players on 2026-09-07 (not "deferred" — won't do). Coup **Reformation** expansion is still deferred (cheap win any time). Reference repos re-validated — see "Reference repos" below; `rumpus/games/{quiplash,fibbage}.js` are the mechanics reference (AGPL → clean-room reimplement, never copy).
...
| Concern | Where |
|---|---|
| Shared UI (`@party/ui`) | `ui/src/tokens.css` (design tokens) + `ui/src/components/` (`BrandMark`, `Button`, `Panel`, `Overlay`, `Timer`, `QrPanel`, `PlayerRoster`, `Avatar`, `AvatarEditor`, `TextAnswerInput`) + `ui/src/components.css` + `ui/src/sound.ts` (synth Web Audio, `getSounds()`). Both CSS files imported in each app's `main.tsx`. Game-specific colours (`--uno-*`) stay in that game's module; per-game sound maps live in `<app>/src/games/<id>/sound-map.ts`. |
...
```
npm run -w server test       # must stay green (currently 120)
...
- History: Fases 5–10 via PR #2 (`feature/testa-claudin`, `c8b602e`). Fases A–C on `feature/coup-ou-coupa` (`22f19b3`). Fase D (Coup) on `feature/coupzin` (`d98f58a`..`7d65968`). Pixel avatars → `0079344`. Zap! (`7ca550a`) + lobby-avatar-reedit / UNO-colour-cancel (`ecfcf00`). **`main`/`develop` are now at `ecfcf00`** (A–D + avatars + Zap! + UX polish). **Current branch `feature/fibbage`** (off `main`, game #4 "Lorota!" — done, not committed/merged yet).

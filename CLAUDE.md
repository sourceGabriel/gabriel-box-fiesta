...
UNO MVP **complete and validated** (Fases 1–10 + mobile visual overhaul). **Coup added as game #2** (Fase D, classic ruleset) with **real character card art** (D6a). **Customizable LPC pixel avatars** added as a platform feature. **Zap! (game #3, Quiplash-inspired) COMPLETE on `feature/quiplaxi`** (Z1–Z5) — shared contract, server engine (77 tests), host view, mobile controller + `@party/ui TextAnswerInput`, sound + reveal animation; full 3-round match smoke-tested end-to-end. **3 games in the catalog.** UX polish (2026-09-08): lobby avatar re-edit (`UPDATE_AVATAR`) + cancelable UNO colour picker. Server tests: **79 green**. Build: 5 workspaces green.
...
**Next-games direction (2026-09-07):** roadmap in `~/.claude/plans/quiplax-e-proximos-jogos.md`. #3 **Zap!** ✅ done (`feature/quiplaxi`) → #4 Fibbage-inspired (reuses `@party/ui TextAnswerInput`) → #5 Trivia. Also parked: a Cards-Against-Humanity-style game (the "everyone answers one shared prompt, vote best" format the user split out from Zap!). Each new game = the §52 checklist (UNO/Coup/Zap! are the mould). **NO BOTS** — the user descoped bots / virtual players on 2026-09-07 (not "deferred" — won't do). Coup **Reformation** expansion is still deferred (cheap win any time). Reference repos re-validated — see "Reference repos" below; `rumpus/games/{quiplash,fibbage}.js` are the mechanics reference (AGPL → clean-room reimplement, never copy).
...
```
npm run -w server test       # must stay green (currently 79)
...
- History: Fases 5–10 via PR #2 (`feature/testa-claudin`, `c8b602e`). Fases A–C on `feature/coup-ou-coupa` (`22f19b3`). Fase D (Coup) on `feature/coupzin` (`d98f58a`..`7d65968`). Pixel avatars → `0079344`. **`main`/`develop` are now at `0079344`** (whole A–D + avatars stack merged). **Current branch `feature/quiplaxi`** (off `main`, for game #3).

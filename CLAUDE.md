...
**Next-games direction (2026-09-08):** roadmap in `~/.claude/plans/quiplax-e-proximos-jogos.md`. #3 **Zap!** ✅ (`feature/quiplaxi`, merged to `main`) → #4 **Lorota!** ✅ (`feature/fibbage`, Fibbage-inspired) → #5 Trivia (next). Also parked: a Cards-Against-Humanity-style game (the "everyone answers one shared prompt, vote best" format the user split out from Zap!). Each new game = the §52 checklist (UNO/Coup/Zap!/Lorota! are the mould). **NO BOTS** — the user descoped bots / virtual players on 2026-09-07 (not "deferred" — won't do). Coup **Reformation** expansion is still deferred (cheap win any time). Reference repos re-validated — see "Reference repos" below; `rumpus/games/{quiplash,fibbage}.js` are the mechanics reference (AGPL → clean-room reimplement, never copy).

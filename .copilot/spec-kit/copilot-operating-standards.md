...
- **Fase C** — 🔜 in progress. Shared `@party/ui` workspace: C1 ✅ (`ui/src/tokens.css` — single source of design tokens, consumed as source by Vite), then C2 components, C3 synthesized Web Audio sounds, C4 consolidate `cardArt.ts`. §47: the "UNO" name is **kept** for now (`BrandMark` parametrized).

...
- **Fase C** — 🔜 in progress. Shared `@party/ui` workspace: C1 ✅ (`ui/src/tokens.css` design tokens), C2 ✅ (components `BrandMark`/`Button`/`Panel`/`Overlay`/`Timer`/`QrPanel`/`PlayerRoster` + retrofit of both apps), then C3 synthesized Web Audio sounds, C4 consolidate `cardArt.ts`. §47: the "UNO" name is **kept** for now (`BrandMark.text` is the single rename point).
